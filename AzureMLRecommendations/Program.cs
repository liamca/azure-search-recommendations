// This application will update usage data to Azure ML Recommendations
...
namespace AzureSearchRecommendations
{
    class Program
    {
        ...
        {
            ExecRecommendations("usage.csv", "M_" + DateTime.Now.Ticks.ToString());
        }

        public static void ExecRecommendations(string UsageFile, string ModelName)
        {
            Console.WriteLine("Invoking Azure ML Recommendations {0}\n", AzureMLRecommendationsEmail);

            //Initialization 
            var sage = new Program();
            sage.Init(AzureMLRecommendationsEmail, AzureMLRecommendationsAccountKey);
            ...
            var report = sage.ImportFile(modelId, "movies.csv", Uris.ImportCatalog);
            Console.WriteLine("\t{0}", report);

            Console.WriteLine("\timport usage...");
            report = sage.ImportFile(modelId, UsageFile, Uris.ImportUsage);
            Console.WriteLine("\t{0}", report);
            ...
            while (monitor)
            {
                status = sage.GetBuildStatus(modelId, buildId);
            ...
            sage.UpdateModel(modelId, "book model", buildId);
            ...
            Console.WriteLine("\nWaiting for 20 sec for propagation of the built model...");
            Thread.Sleep(20000);
            ...
            var seedItems = new List<CatalogItem>
            {
                ...
                new CatalogItem() {Id = "761", Name = "Star Wars: Episode I - The Phantom Menace"}
            };
        ...
        public static class Uris
        {
            ...
            public const string DeleteModelUrl = "DeleteModel?modelId=%27{0}%27&apiVersion=%271.0%27";
